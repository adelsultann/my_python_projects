...
class Ball(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        # to make the ball default position in the half of the screen
        ...
        # generate random velocity so we change the ball movement
    ...
    def update(self):
        ...
        if self.y_pos < 11:  # top of the screen | in case of Y position we only care about the top not bottom
            self.velocity[1] = -self.velocity[1]

            # Prevent velocities from getting stuck at 0.
            # احيانا بتكون على الصفر في شكل افقي وثابت في هالحاله بنضيف ثيمه عشان ماتعلق في النص
        if self.velocity[0] == 0:
            self.velocity[0] += 1
        if self.velocity[1] == 0:
            self.velocity[1] += 1
        # finally Update position based on velocity.
